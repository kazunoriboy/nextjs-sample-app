...
const Button = styled.button<ButtonProps>`
  ${({ variant, color, backgroundColor, pseudoClass, theme }) => {
    if (variant && variants[variant]) {
      const styles = []
      !color && 
        styles.push(toPropValue('color', variants[variant], theme))
      !backgroundColor &&
  ...
  ${(props) => toPropValue('min-height', props.minHeihgt, props.theme)}
  ...
  &:disabled {
    ${(props) => toPropValue(
      'background-color',
      props?.pseudoClass?.dispabled?.backgroundColor
    )}
  }
  cursor: pointer;
  outline: 0;
